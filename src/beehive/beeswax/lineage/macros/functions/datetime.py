...
def truncate_timestamp(source, granularity: lineage_values.Interval):
    ...
    return lineage_functions.Add(
        left=lineage_functions.TruncateTimestamp(
            source=source,
            granularity=lineage_values.Varchar(unit_shift[granularity.unit.value]),
        ),
        right=lineage_functions.Multiply(
            left=lineage_functions.Cast(
                source=lineage_functions.Divide(
                    left=lineage_functions.DatePart(
                        source=source,
