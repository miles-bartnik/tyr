...
def select_primary_key(
    table: lineage._Table, filter_unit: units.core.Unit = None, filter_regex: str = None
):
    ...
    select_columns = lineage.ColumnList(
        [
            ...
            for column in table.primary_key.list_columns_(
...
def select_static_primary_key(
    ...
    select_columns = lineage.ColumnList(
        [
            ...
            for column in table.static_primary_key.list_columns_(
                filter_regex=filter_regex, filter_unit=filter_unit
            )
        ]
    )
...
def select_all(
    ...
    if primary_key and not table.primary_key.is_empty:
    ...
    elif static_primary_key and not table.static_primary_key.is_empty:
        if apply_filters_to_primary_key:
    ...
    select_columns = lineage.ColumnList(
        [
            ...
            for column in table.columns.list_columns_(
                filter_regex=filter_regex, filter_unit=filter_unit
            )
            if column.name not in table.primary_key.list_names_()
        ]
    )
